...
server <- function(input, output) {
  ...
  output$find_crime_from_month <- renderPlot({
    ...
    index_of_type <- which(data_of_the_month$offense == input$crime_type)
    ...
    ggmap(map) + ggplot2::geom_jitter(data = data_of_type, ggplot2::aes(x=lon, y=lat),size = 0.005, color = "red")
